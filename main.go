...
func worker() {
	...
	for !stopFlag {
		...
		// 1. CONNECT tunnel ke target:443
		...
		// 3. HTTP/2 client manual (tanpa dependency)
		// Karena stdlib Go butuh http2 transport, kita pake http.Client dengan custom dialer + TLS
		// Tapi lebih stabil pake http2.Transport bawaan (tanpa import eksternal)
		tr := &http.Transport{
...
func main() {
	...
	if err != nil {
		fmt.Println("[!] Target invalid")
		os.Exit(1)
	}
	if targetURL.Scheme != "https" {
		fmt.Println("[!] Hanya support https")
		os.Exit(1)
	}
