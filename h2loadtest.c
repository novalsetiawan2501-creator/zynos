/*
 * h2loadtest.c — HTTP/2 load testing tool (C + nghttp2 + pthread + OpenSSL)
 *
 ...
 *   gcc -O2 -o h2loadtest h2loadtest.c -lnghttp2 -lpthread -lssl -lcrypto
 */
...
static SSL_CTX *create_ssl_ctx(void) {
  ...
  if (!ctx) return NULL;
  /* Load tester internal: verifikasi sertifikat dimatikan supaya bisa
     menguji endpoint staging/self-signed juga. Aktifkan kembali bila perlu. */
...
static int submit_request(connection_t *conn) {
  ...
  nghttp2_nv hdrs[] = {
      ...
      MAKE_NV("accept-language", "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"),
