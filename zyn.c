/*
 * hzx.c — HTTP/2 load testing tool (C + nghttp2 + pthread + OpenSSL)
 *
 ...
 *   gcc -O2 -o h2loadtest hzx.c -lnghttp2 -lpthread -lssl -lcrypto
 */
...
/* ================= rate limiter (token bucket) - FIXED ================= */

static void *rate_ticker_fn(void *arg) {
  (void)arg;
  const long tick_ms = 1;
  long per_tick = (long)g_config.rate * tick_ms / 1000;
  if (per_tick < 1 && g_config.rate > 0) per_tick = 1;
  long burst_cap = g_config.rate * 2;
  if (burst_cap < 1000) burst_cap = 1000;
...
static int try_consume_token(void) {
  if (g_config.rate <= 0) return 1;
  long cur = atomic_load(&g_rate_tokens);
  while (cur > 0) {
    if (atomic_compare_exchange_weak(&g_rate_tokens, &cur, cur - 1)) return 1;
  }
  
  if (atomic_load(&g_rate_tokens) == 0) {
    long expected = 0;
    if (atomic_compare_exchange_strong(&g_rate_tokens, &expected, 1)) {
      return 1;
    }
  }
  return 0;
...
static int init_connection(connection_t *conn) {
  ...
  int rv = connect(fd, (struct sockaddr *)&g_target_addr, g_target_addrlen);
...
static void setup_http2_session(connection_t *conn) {
  ...
  nghttp2_submit_settings(conn->session, NGHTTP2_FLAG_NONE, iv, 6);
...
int main(int argc, char **argv) {
  ...
  int opt;
  while ((opt = getopt_long(argc, argv, "u:r:t:c:w:d:h", long_opts, NULL)) != -1) {
    switch (opt) {
    case 'u':
